...
2. Import the sveltify package and pass it three arguments: 

    i. **svelte component** (required)

    ii. **id of the element where you want to insert svelte**. (required and must of unique for every sveltify function call)
...
import { SveltifyComponent, sveltify } from "sveltify"
import Test from "./Test.svelte";

export default function Sample() {
    const [count, setCount] = useState(0)

    let sc: SveltifyComponent;

    sveltify(Test, "test-id",{
        props: {
            firstProp: "test string",
            counter: count 
        }   
    }).then((res: SveltifyComponent)=>{
        sc = res;
    })
...
const activateSveltify=()=> {
  sveltify(Test, "vue-svelte-container", {
  props: {
...
activateSveltify(); // you can call it inside onMounted too. It's your choice.
...
In case of vue, when you update the vue states, ```activateSveltify()``` won't be called again, unlike react where we just placed the function call inside the component, and it worked. So, we need to re-trigger it on component update i.e. ```onUpdated()```. 


**Don't worry, sveltify won't create new object and overload the memory. It will just update the props. That is why the id where the svelte element is inserted must be unique for every sveltify function call.**
...
| options | SveltifyComponentOpts   | **Third Argument** Addtional options to pass. You can only pass props object for this initial release. **props** are the props required for your svelte component.   |
